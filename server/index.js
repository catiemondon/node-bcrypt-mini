...
app.post('/auth/signup', async (req, res)=>{
  ...
  //Sees if user already exists by email
  const foundUser= await db.check_user_exists(email)
  if(foundUser[0]) return res.status(409).send('Email taken')

//Sets up salt, then uses salt in combination with password to hash the password
  const salt= bcrypt.genSaltSync(10)
  const hash= bcrypt.hashSync(password, salt)

  //Uses  create_user sql method to insert new email and hash into database, then returns those things (as written in .sql file)
